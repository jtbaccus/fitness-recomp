...
export default function GroceryPage() {
  ...
  const loadIngredients = useCallback(async () => {
    ...
    const allRecipes: (Recipe & { ingredients?: Array<{ ingredient?: Ingredient }> })[] = await res.json();
    ...
    // Batch recipes for banner
    setBatchRecipes(allRecipes.filter(r => r.freezer_friendly));
  }, []);
  ...
  return (
    <PageShell title="Grocery List" subtitle={`Week of ${weekStart}`}>
      <div className="space-y-4">
        <Link href="/plan" className="text-xs text-accent">← Back to Calendar</Link>
